...
## Monorepo o múltiples paquetes (backend + frontend)

Antes de escribir schemas, **verificar la versión de Zod** en el `package.json` del paquete donde vivirá el código:

| Contexto | Regla |
|----------|--------|
| **Backend** (p. ej. `env.schema.ts`, webhooks, pipes) | Usar la API de la major instalada (Zod 4: `z.email()`, `z.uuid()`, `z.url()`, `z.string().min(1)` en lugar de `.nonempty()`, `{ error: "…" }` en mensajes). Los **DTOs Nest** suelen seguir con **class-validator** salvo un pipe Zod explícito. |
| **Frontend** (forms, `src/schemas/**`) | Puede estar en otra major que el backend; no mezclar APIs v3/v4 en el mismo paquete. En Zod 3: `z.string().email()`, refinements con `{ message: "…" }`. Preferir **`safeParse`** en payloads de red. |

Si backend y frontend comparten monorepo, documentar en el repo cuándo migrar cada paquete a la misma major.
