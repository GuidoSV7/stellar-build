...
## Cumplimiento en este repositorio

| Tema | Regla |
|------|--------|
| **Versión Tailwind** | Verificar en `package.json`. La skill cubre patrones compatibles con **v3 y v4**; `@theme` CSS aplica al migrar a v4. |
| **Colores** | Priorizar tokens en `tailwind.config.ts` → `theme.extend.colors`. **Evitar** `bg-[#…]` / `text-[#…]` cuando exista token semántico equivalente. |
| **`var()` en `className`** | Evitar salvo deuda heredada. Sustituir por clases semánticas; si el color es solo de **runtime** (API), usar **`style={{ color: … }}`** o variables CSS en `style` **sin** `var()` en `className`. |
| **`cn()`** | Usar helper del repo (p. ej. `lib/cn.ts` con `clsx` + `tailwind-merge`). Import según convención del proyecto. Usar con condicionales, estados o props `className`; no envolver clases 100 % estáticas. |
| **CSS global de terceros** | Hex en `globals.css` para editores o libs son aceptables si no son utilidades Tailwind en JSX. |

**Validación:** en componentes nuevos, buscar arbitrarios `\[[\]#[0-9a-fA-F]{3,8}\]` en `*.tsx` y sustituir por tokens del tema.
